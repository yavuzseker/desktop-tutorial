...
import { incramentReading } from "src/store";

import { useSelector, useDispatch } from "react-redux"
import { RootState } from "src/store"

import { TextArea, Difficult, AnswerStep, LearnBottomButton } from "src/components";
    ...
    React.useEffect(() => {
        ...
        if (route.params) {
            ...
            learn.dailyQuests({
                ...
                setLoading(false)
                setNews(res[learnState.studyComplate.reading])
            })
        }
    }, [])
            ...
            <View style={{ flex: 1, backgroundColor: "#fff" }}>
                ...
                <ScrollView style={{ flex: 1, padding: 12, backgroundColor: "#fff" }}>

                    {
                        step == 1 ?
                            (
                                <TextArea data={news} visibleLink={true} />)
                            :
                            step == 2 ? (
                                <Difficult data={news} />) :
                                (<AnswerStep data={news} incrament={incramentReading} navigation={navigation} route={route} />)
                    }
